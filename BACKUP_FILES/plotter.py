...
class Plot:
    ...
    def __init__(self, data: Data, mode: str = None):
        ...
        self.ne_mCount, self.bs_mCount = 0, 0
        # AXIS TITLES
    ...
    def setMode(self, mode: str):
        if mode == 'ne':
            ...
            self.fig, ((self.axNe,self.axTemp), (self.axNeGrad,self.axTempGrad), (self.axBs,self.axB)) = plt.subplots(nrows=3, ncols=2, sharex=True, figsize=(10, 9))
    ...
    def setIterations(self, number: int = 1):
        ...
        # is needed for correct couler scheming of manipulated profiles
    ...
    def setAxisB(self):
        self.axB.set_xlabel(xlabel=self.AXIS_RHO)
        self.axB.set_ylabel(ylabel=self.AXIS_B)
    ...
    def addB(self, b: list):
        self.axB.plot(self.rho, b, color=self.COLOR_TI, label=self.LABEL_B)
        self.setAxisB()
    ...
    def addBsPeeters(self, bs_p: list):
        if not self.bsMode:
            return
        self.axBs.plot(self.rho, bs_p, color=self.COLOR_BS, label=self.LABEL_BS_P, linestyle=self.LINESTYLE_BS)
        self.setAxisBs()

    def addBsRedl(self, rho_r: list, bs_r: list):
        if not self.bsMode:
            return
        # Interpolate data first to get same length 200
        new_x = np.linspace(0, self.rho[-1], len(self.rho))
        new_y = np.interp(new_x, rho_r, bs_r)
        self.axBs.plot(new_x, new_y, color=self.COLOR_BS_M, label=self.LABEL_BS_R, linestyle=self.LINESTYLE_BS_M)
        self.setAxisBs()
    ...
    def save(self, filename: str = None):
        ...
        self.fig.savefig(fname=f'/afs/ipp/home/s/slech/Documents/Plots/{filename}.pdf', dpi=200,format='pdf', bbox_inches='tight') #png
    ...
    def setLegend(self):
        ...
        if self.bsMode:
            ...
            self.axB.legend(fontsize=self.LEGEND_FONTSIZE, facecolor=self.COLOR_LEGENDBOX)
